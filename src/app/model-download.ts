...
export async function decodeModelResponse(response:Response,expectedBytes:number,compressed:boolean):Promise<ArrayBuffer>{
 if(!response.ok)throw new Error('Bir anatomi dosyası yüklenemedi.');
 ...
 if(buffer.byteLength!==expectedBytes)throw new Error('Bir anatomi dosyası eksik indirildi. Lütfen görüntüleyiciyi yeniden yükleyin.');
 return buffer;
}
