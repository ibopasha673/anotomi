...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  scene.add(new T.HemisphereLight(0xffffff,0x3d4d63,1.05));
  const key=new T.DirectionalLight(0xfffaf4,2.3);key.position.set(-2,4,3);scene.add(key);
  const rim=new T.DirectionalLight(0x8fc9ff,1.7);rim.position.set(2,2,-3);scene.add(rim);
  const fill=new T.DirectionalLight(0xffb98a,.4);fill.position.set(0,-1,2.5);scene.add(fill);
  ...
  const overallBounds=new T.Box3();bounds.forEach(b=>overallBounds.union(b));
  const clipPlane=new T.Plane();let lastCross='',crossActive=false,lastHighlight:string|null='';
  const highlightColor={value:new T.Color(0x6bd9c7)};
  const pulseAmount={value:0};
  let packingWidth=1,packingHeight=1,baseDistance=4,lastVisibility='';const baseCenter=new T.Vector3(0,.85,0);
  ...
  const vividColor=(hex:string)=>{const c=new T.Color(hex),hsl={h:0,s:0,l:0};c.getHSL(hsl);return c.setHSL(hsl.h,Math.min(1,hsl.s*1.4+.08),T.MathUtils.clamp(hsl.l*1.03,0,.92));};
  const materialFor=(system:string)=>{
   const m=new T.MeshPhysicalMaterial({color:vividColor(SYSTEMS.find(s=>s.id===system)?.color??'#aebbb8'),metalness:.05,roughness:.44,clearcoat:.18,clearcoatRoughness:.35,side:T.DoubleSide,transparent:system==='integumentary',opacity:system==='integumentary'?.1:1,depthWrite:system!=='integumentary'});
   m.onBeforeCompile=shader=>{
    shader.uniforms.partState={value:partTexture};shader.uniforms.selectionState={value:selectionTexture};shader.uniforms.stateWidth={value:width};shader.uniforms.highlightColor=highlightColor;shader.uniforms.pulseAmount=pulseAmount;
    shader.vertexShader='attribute float partIndex; uniform sampler2D partState; uniform sampler2D selectionState; uniform float stateWidth; varying float partVisible; varying float partSelected;\n'+shader.vertexShader;
    shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nvec2 stateUv = vec2((partIndex + 0.5) / stateWidth, 0.5); vec4 state = texture2D(partState, stateUv); transformed += state.xyz; partVisible = state.w; partSelected = texture2D(selectionState, stateUv).r;');
    shader.fragmentShader='uniform vec3 highlightColor; uniform float pulseAmount; varying float partVisible; varying float partSelected;\n'+shader.fragmentShader;
    shader.fragmentShader=shader.fragmentShader.replace('#include <clipping_planes_fragment>','#include <clipping_planes_fragment>\nif (partVisible < 0.5) discard;');
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\ndiffuseColor.rgb = mix(diffuseColor.rgb, highlightColor, partSelected * 0.7);');
    shader.fragmentShader=shader.fragmentShader.replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\ntotalEmissiveRadiance += highlightColor * partSelected * pulseAmount * 0.85;');
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('Anatomi geometrisi birleştirilemedi.');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:'Anatomi yüklenemedi.');}})();
  const fit=(view:string,extent=0)=>{
   const aspect=camera.aspect,mobile=el.clientWidth<768,normalDistance=mobile?Math.max(4.5,baseDistance*1.15):baseDistance;
   ...
   controls.target.set(extent>.1?(el.clientWidth>767?-packingWidth*.12:0):baseCenter.x,extent>.1?.85:baseCenter.y,extent>.1?0:baseCenter.z);camera.position.copy(controls.target).addScaledVector(direction,distance);controls.update();dirty=true;
  };
  ...
  const animate=()=>{
   ...
   if(s.selected.length&&!s.isolate){pulseAmount.value=Math.sin(performance.now()*.0022)*.5+.5;dirty=true;}else if(pulseAmount.value!==0){pulseAmount.value=0;dirty=true;}
   const changed=lastState?.visible!==s.visible||lastState?.selected!==s.selected||lastState?.isolate!==s.isolate||lastState?.cross!==s.cross||lastState?.crossAmount!==s.crossAmount||lastState?.crossInvert!==s.crossInvert;
   const moving=Math.abs(amount-s.explode)>.0001;
   const visibilityKey=`${s.visible.join(',')}:${s.isolate}`;
   if(visibilityKey!==lastVisibility){
    lastVisibility=visibilityKey;
    if(!s.isolate){
     const box=new T.Box3();atlas.parts.forEach((p,i)=>{if(s.visible.includes(p.system))box.union(bounds[i]);});
     if(!box.isEmpty()){
      const size=box.getSize(new T.Vector3()),mobile=el.clientWidth<768,reservedHeight=mobile?350:270;
      const availableAspect=Math.max(.35,(el.clientWidth-(mobile?40:340))/Math.max(160,el.clientHeight-reservedHeight));
      baseDistance=Math.max(.3,Math.max(size.y,size.x/availableAspect,size.z)/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*1.35);
      box.getCenter(baseCenter);
     }
     if(amount<.05)fit(s.view,0);
    }
   }
   const crossKey=`${s.cross}:${s.crossAmount}:${s.crossInvert}:${s.selected.join(',')}`;
   if(crossKey!==lastCross){
    crossActive=s.cross!=='none';
    if(crossActive){
     let box=overallBounds;
     if(s.selected.length){
      const sel=new T.Box3();atlas.parts.forEach((p,i)=>{if(s.selected.includes(p.id))sel.union(bounds[i]);});
      if(!sel.isEmpty()){sel.expandByVector(sel.getSize(new T.Vector3()).multiplyScalar(.12));box=sel;}
     }
     const onY=s.cross==='top',axis=onY?box.min.y:box.min.x,extent=onY?box.max.y:box.max.x;
     const level=T.MathUtils.lerp(axis,extent,s.crossAmount),normal=new T.Vector3(onY?0:-1,onY?-1:0,0);
     if(s.crossInvert)normal.negate();
     clipPlane.setFromNormalAndCoplanarPoint(normal,onY?new T.Vector3(0,level,0):new T.Vector3(level,0,0));
    }
    lastCross=crossKey;dirty=true;
   }
   if(s.highlightColor!==lastHighlight){highlightColor.value.set(s.highlightColor??0x6bd9c7);lastHighlight=s.highlightColor;dirty=true;}
   ...
   if(changed||moving||lastExtent<0){
    ...
    if(nextLayoutKey!==layoutKey){const layout=createExplosionLayout(visibleParts,camera.aspect);packingWidth=layout.width;packingHeight=layout.height;atlas.parts.forEach((p,i)=>{const cell=layout.cells.get(p.id);offsets[i]=cell?new T.Vector3(cell.x,cell.y+.85,0):centers[i].clone();});layoutKey=nextLayoutKey;controls.maxTargetRadius=Math.max(5,packingWidth,packingHeight)*.75+3;if(amount>.05&&!s.isolate)fit(s.view,Math.max(0,(amount-.3)/.7));}
    ...
    atlas.parts.forEach((p,i)=>{
     ...
     const crossKept=!crossActive||(clipPlane.normal.x*(c.x+dx)+clipPlane.normal.y*(c.y+dy)+clipPlane.normal.z*(c.z+dz)+clipPlane.constant>=0);
     data.set([dx,dy,dz,(baseVisible&&crossKept)?1:0],i*4);selectedData[i*4]=selected?255:0;
